...
- 在会话面板中显示未读数和最近消息摘要
- 进入会话时清除该会话的未读和提及标记；当前会话收到的新消息不累计待处理计数
...
输入区支持多行草稿和自动折行，最多显示 5 行，超出后视口跟随光标滚动。粘贴的多行文本会保留换行，按 Enter 统一发送。

快捷键：

| 快捷键 | 说明 |
| --- | --- |
| `Enter` / `Shift+Enter` | 发送消息 / 在光标处换行 |
| `Esc` | 关闭面板或清空输入 |
| `Tab` / `Enter`（候选展开时） | 接受当前补全；支持命令、会话、消息 ID 和音频路径 |
| `↑` / `↓`（候选展开时） | 选择候选；回复和转发候选显示作者及消息预览 |
...
补全在纯文本命令的输入末尾自动显示，最多占用 5 行。补全命令后可继续选择参数；完整命令和别名（如 `/groups`、`/g`）按 Enter 直接执行；输入尚未补完时，Enter 先填入候选，再按 Enter 执行。Tab 始终用于补全。移动光标到输入中间会收起补全。
...
开启 `/images inline` 后，GIF 使用静态首帧预览，沿用终端支持的高清图片协议。
消息列表不解码动画帧、不启动播放定时器，以避免逐帧重绘引起闪烁和输入卡顿。
动画目前仅用于独立实验：运行 `npm run gif:demo`
可查看本地演示，也可以用 `npm run gif:demo -- /绝对路径/example.gif` 预览自己的文件。
演示中按 `h` 对比自动高清和色块模式，按 `q` 或 Esc 退出。

当前播放间隔最短为 100ms（较快的 GIF 会放慢），组件卸载时停止播放。
动画帧最多保留 560×400 像素。超过 300 帧、单帧 400 万像素、总解码 4000 万像素
或编码帧缓存 16 MiB 的 GIF 使用静态预览。裁切中的图片沿用色块模式。
